...
def modelar_y_visualizar_arbol(csv_file,cantidad,tracker):
    try:
        ...
        if numero_juego != None:
            ...
            for _ in range(posicion_deseada):
                try:
                    ...
                    break
            index, fila_seleccionada = next(iterador_filas)
            x = pd.concat([pd.DataFrame([fila_seleccionada], columns=x.columns), x], ignore_index=True)
            y = pd.concat([pd.Series([fila_seleccionada[y.name]], name=y.name), y], ignore_index=True)
            print(x)
            print(y)
...
class ActionUsarModelo(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            modelo_arbol = modelar_y_visualizar_arbol(gusto_csv,cantidad,tracker)
...
class CambiarGustoJuego2(Action):
    def name(self):
        return "action_cambiar_gusto_juego2"
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if nombre_juego != "Juego no encontrado":
                ...
                print(indice_juego)
                return [SlotSet("numero_juego", indice_juego), SlotSet("juego", nombre_juego)]
            else:
